...
rule {
    ...
    value {
        ...
        when {
            [Industry.ID_ARMY, Industry.ID_ART, Industry.ID_MEDECINE, Industry.ID_SECURITY].contains(x)
        } then '0.50416979'
        when {
            [Industry.ID_GOV_SERVICE, Industry.ID_IT, Industry.ID_OTHER, Industry.ID_SALES,
             Industry.ID_SCIENCE, Industry.ID_SERVICE].contains(x)
        } then '0.105224359'
        when {
            [Industry.ID_BOOKKEEPING, Industry.ID_BUILDING, Industry.ID_FEC,
             Industry.ID_PRODUCTION, Industry.ID_RAWMATERIALSMINING, Industry.ID_TRANSPORT].contains(x)
        } then '-0.576053246'
...
rule {
    multiplier '-0.7872'
    x 'fcbGcvpEmployersCount3Months'
    value {
        when { missing(x) } then '0.666611627'
        when { x < 1 } then '0.666611627'
        when { x in 1..<2 } then '-0.025907401'
        when { x >= 2 } then '-0.792607974'
    }
}
